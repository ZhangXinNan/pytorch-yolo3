...
def load_conv(buf, start, conv_model):
    ...
    print('conv_model.weight.shape : ', conv_model.weight.shape)
    conv_model.weight.data.copy_(torch.reshape(torch.from_numpy(buf[start:start+num_w]),conv_model.weight.shape)); start=start + num_w
    return start
...
def load_conv_bn(buf, start, conv_model, bn_model):
    ...
    # conv_model.weight.data.copy_(torch.from_numpy(buf[start:start+num_w])); start = start + num_w
    # conv_model.weight.data.copy_(torch.from_numpy(buf[start:start+num_w]).view(conv_model.weight.data.shape));   start = start + num_b
    print('conv_model.weight.shape : ', conv_model.weight.shape)
    conv_model.weight.data.copy_(torch.reshape(torch.from_numpy(buf[start:start+num_w]),conv_model.weight.shape)); start=start + num_w
    return start
